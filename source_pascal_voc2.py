...
#from utils import rgb2bgr, abs2propSA
...
#-------------------------------------------------------------------------------
label_defs = [
    Label('text',   rgb2bgr((255,     0,   0)))]
   

#---------------------------- function to read different dataset with different way ----------------------
def MSRA_DS_Reading():
    Input_Loc = ('E:/#gp/MSRA-TD500/necortest65/anntestCopy/')  # n3'er doool 
    Directory=os.listdir(Input_Loc)
    Dataset_List = list()
    Image_Words=list()
    ImageDirctory= ('E:/#gp/MSRA-TD500/necortest65/testCopy/')  # w de 
    label = 'text'
    samples=[]
    for i,FileName in enumerate(Directory):
        ImageName = FileName[0:8]
        ThisImage = cv2.imread(ImageDirctory + ImageName+".JPG")
        ImageSize = Size(ThisImage.shape[1], ThisImage.shape[0])
        boxes=[]
        FileGt=open(Input_Loc+FileName,"r")
        ...
        for line in FileLines:
            ...
            box = Box(label, 0, center, size)
            boxes.append(box)
        sample = Sample(ImageDirctory + ImageName+".JPG", boxes, ImageSize)
        samples.append(sample)
   # print(samples[0])
    return  samples

def SVT_DS_ReadingTrain1():
 
    label='text'
    File =minidom.parse('E:\\#gp\\svt\\svt1\\train.xml')
    ImgName = File.getElementsByTagName('imageName')
    taggedRectangles=File.getElementsByTagName('taggedRectangles')
 
    ImagesDir="E:\\#gp\\svt\\svt1\\img\\"
    samples = []
    boxes=[]
    

    for i,elements in enumerate(taggedRectangles):
         BoundryBox=elements.getElementsByTagName('taggedRectangle')
         ImageName=ImgName[i].firstChild.data
         ImageName=ImageName[4:len(ImageName)]
         ThisImg=cv2.imread(ImagesDir+ImageName)
         ImgSize= Size(ThisImg.shape[1], ThisImg.shape[0])

         # print("-------------->",ImageName)

         for j,Record in enumerate (BoundryBox):

             height=int(Record.attributes['height'].value)
             width=int(Record.attributes['width'].value)
             xmin=int(Record.attributes['x'].value)
             ymin=int(Record.attributes['y'].value)
             xmax=width-xmin
             ymax=height-ymin 
             center, size = abs2prop(xmin, xmax, ymin, ymax, ImgSize)
             box = Box(label, 0, center, size)
             boxes.append(box)
         sample = Sample(ImageName, boxes, ImgSize)
         samples.append(sample)
            
    return samples


def SVT_DS_ReadingTrain2():
   
    label='text'
    File =minidom.parse('E:\\#gp\\svt\\svt1\\test.xml')
    ImgName = File.getElementsByTagName('imageName')
    taggedRectangles=File.getElementsByTagName('taggedRectangles')
    ImagesDir="E:\\#gp\\svt\\svt1\\img\\"
    samples = []
    ...
    for i,elements in enumerate(taggedRectangles):
         ...
         for j,Record in enumerate (BoundryBox):
             ...
             width=int(Record.attributes['width'].value)
             xmin=int(Record.attributes['x'].value)
             ymax=int(Record.attributes['y'].value)
             ymin=ymax+height
             xmax=width+xmin
             center, size = abs2prop(xmin, xmax, ymin, ymax, ImgSize)
             box = Box(label, 0, center, size)
...
class PascalVOCSource:
    ...
    def __build_annotation_list(self,root):#
        ...
        for name,j in enumerate(files):
             file = open(annot_root+j,'r')
    ...
    def __build_sample_list(self, root, annot_files):
        ...
        for fn in tqdm(annot_files, unit='samples'):
                ...
                file = open(fn.name,'r', encoding="utf8")
                ...
                for obj in objects:
                    ...
                    #-----------------------------------------------------------
                    label = 'text'
                    xmin  = int(float(obj.xpath('polygon/pt/x')[0].text))
                   
                    xmax  = int(float(obj.xpath('polygon/pt/x')[2].text))
                    ymin  = int(float(obj.xpath('polygon/pt/y')[0].text))
                    
                    ymax  = int(float(obj.xpath('polygon/pt/y')[2].text))
                    center, size = abs2prop(xmin, xmax, ymin, ymax, imgsize)
                    box = Box(label, 0, center, size)
                    boxes.append(box)
                if not boxes:
    ...
    def load_trainval_data(self, valid_fraction):
        ...
        train_annot = []
        train_samples = []
        root='E:\\#gp\\gp\\neocr_dataset\\neocr_dataset\\Images\\users\\pixtract\\dataset\\'

        root2='E:\\#gp\\gp\\neocr_dataset\\neocr_dataset\\Annotations\\users\\pixtract\\dataset\\'
        annot = self.__build_annotation_list(root2)

        train_samples = self.__build_sample_list(root, annot)
        train_samples += SVT_DS_ReadingTrain1()
        train_samples += MSRA_DS_Reading()

        root = 'E:\\#gp\\gp\\neocr_dataset\\neocr_dataset\\valimage\\'
       
        root2='E:\\#gp\\gp\\neocr_dataset\\neocr_dataset\\valannotation\\'
        valid_annot = self.__build_annotation_list(root2)
    ...
    def load_test_data(self):
        ...
        #self.test_samples += MSRA_DS_Reading()                              # lw darb w2fy da
...
####3shan nrano
#PascalVOCSource.load_test_data(get_source())      ########### for debugging
